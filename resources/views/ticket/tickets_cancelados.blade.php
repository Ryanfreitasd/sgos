...
@section('title', 'Ordens Canceladas')
...
<div class="card-header">
  <i class="fas fa-table"></i><strong> ORDENS DE SERVIÇO CANCELADAS </strong></div>
<div class="card-body">
  <div class="table-responsive">
    ...
    <div class="card-footer">
       <button class="btn btn-primary btn-sm" onclick="setaTicket()" disabled>Nova ordem</button>
    </div>
  </div>
</div>
</div>
...
<div class="modal" tabindex="-1" role="dialog" id="dlgTicket">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <form class="form-horizontal" id="formTicket" readonly>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6 ">
                                ...
                                <select class="form-control" id="nomeClienteTicket" required readonly="false"></select>
                            </div>                        
                            ...
                            <div class="form-group col-md-3 ">
                                ...
                                <input type="hidden" class="form-control" id="dataEntrega" readonly="false">
                            </div> 
                        </div>
                        <div class="form-row">
                            <div class="form-group col-md-5 ">
                                <label for="usuarioTicket" class="control-label">Usuário</label>
                                <select class="form-control" id="usuarioTicket"  readonly="false"></select>
                            </div> 
                            <div class="form-group col-md-4 ">
                                <label for="prioridadeTicket" class="control-label">Prioridade</label>
                                <select class="form-control" id="prioridadeTicket" required readonly="false"></select>
                            </div>           
                            <div class="form-group col-md-3 ">
                                <label for="statusTicket" class="control-label">Status</label>
                                <select class="form-control" id="statusTicket" required readonly="false"></select>
                            </div>
                        </div>  
                        <div class="form-group">
                            <label for="observacaoTicket" class="control-label">Conteúdo</label>
                        <div class="input-group">
                            <textarea class="form-control" id="observacaoTicket" rows="5" required readonly="false"></textarea>
                        </div>
                        </div>
                </div>
...
@section('javascript')
    <script type="text/javascript">
        ...
        function salvarTicket() { // salva o produto
            tic = { 
                id:                 $("#id").val(),
        ...
        function carregarTicket(){ // CARREGA AS INFORMAÇÕES NA TABELA PRINCIPAL
          $.getJSON('/api/tickets_cancelados', function(ticket){
        ...
        function criarTicket() { // CRIA O PRODUTO RECEBENDO AS INFORMAÇÕES DO FORMULARIO
            ...
            $.post("/api/tickets", tic, function(data) {
     ...
     //ABRI O FORMULÁRIO PARA EDITAR AS INFORMAÇÕES   
        function editar(id){ 
            $.getJSON('/api/tickets/'+ id, function(data){
                if(data.status_id == 5){
                    ...
                    $('#dataEntrega').val(data.delivery_date);
        ...
        // função para bloquear os tickets concluidos
        var input = document.querySelectorAll('.form-control');
        
        function bloquear(){
            for(i=0; i< input.length; i++){
                if ($("#statusTicket").val() == 5 && $("#statusTicket").val() == 4)
